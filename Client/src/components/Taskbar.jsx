...
const NAV_SECONDARY = [
  ...
  { to: '/inventory', icon: Package, label: 'Inv', match: (p) => p.startsWith('/inventory') },
...
export default function Taskbar() {
  ...
  return (
    <nav className="taskbar glass-bg" aria-label="Mobile navigation">
      <div className="taskbar-inclides">
        <InclidesBalancePill compact />
      </div>
      <div className="taskbar-rows">
        <div className="taskbar-links">
          {NAV_PRIMARY.map(({ to, icon: Icon, label, match }) => {
        ...
        <div className="taskbar-links taskbar-links--secondary">
          {NAV_SECONDARY.map(({ to, icon: Icon, label, match }) => {
            const active = match(location.pathname);
            return (
              <NavLink
                key={to}
